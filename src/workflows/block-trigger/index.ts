...
const initWorkflow = (config: Config) => {
  ...
  const onLogTrigger = (runtime: Runtime<Config>, payload: EVMLog): string => {
    runtime.log('Running block-trigger workflow')
    ...
    if (!payload.blockNumber) throw new Error('Block number is required')
    ...
    const headerResponse = evmClient
      .headerByNumber(runtime, { blockNumber: bigintToProtoBigInt(blockNumber) })
      .result()
